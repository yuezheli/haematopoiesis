...
modo <- mread("erythrocytes_Hb_lymphoid_myeloid") %>% param(alphaCLP2DN = 0)
...
sampleperparam = 500 # sampling size per parameter, for sobol2007
...
setmppclp <- c('alphaMPP2CLP', 'alphaCLP2proB', 'betaCLP', 'kCLP')
setproB <- c('CLP2proB_amp') # both are guessed parameters, thus with the most uncertainty

N <- sampleperparam * length(c(setmppclp,setproB))

# set different variation in each group
sets <- list(
  list(which = setmppclp, factor = c(0.9, 1.1), N = N, l = param(mod_mppt)), 
  list(which = setproB, factor = c(0.25,4), N = N, l = param(mod_mppt))
)

##---------------------------------- set up global sensitivity analysis using Sobel method ----------------------------------##
...
batch_MPPT <- function(x) {
  ...
  mod_mppt %>% 
    ...
    na.omit() %>%
    group_by(ID) %>% 
    summarise(bexp = auc_partial(time, transB)) %>% 
    pull(bexp)
}
...
if(TRUE) # note this section can be computational intensive; don't run unless you have a cluster
{
  ...
  saveRDS(sobolx, file = "SobolData_B.rds")
}
...
x <- readRDS("SobolData_B.rds")
### plot
globSens <- tibble(Parameter = c(setmppclp,setproB),
                   main = x$S$original,
                   main_lo = x$S$'min. c.i.',
                   main_hi = x$S$'max. c.i.',
                   total = x$T$original,
...
#png('GlobalSens_MPPT_B.png', width = 800, height = 300)
#print(fig_sens_glob)
#dev.off()
}
