...
mod_clpt = global_clpt()

## define the output function
CMaxTime <- function(time, transT)
{
  # this is to find the when Cmax shows up 
 tmp_time = time[which(transT == max(transT))[1]]
 return(tmp_time)
}
...
sets <- list(
  list(which = setmppclp, factor = c(0.9, 1.1), N = N, l = param(mod_clpt)), 
  list(which = setT, factor = c(0.25,4), N = N, l = param(mod_clpt))
)

##---------------------------------- set up global sensitivity analysis using Sobel method (MPP transplant) ----------------------------------##

# CLPT, the time for maximum T ratio
batch_CLPT <- function(x) {
  ...
  mod_clpt %>% 
    ...
    na.omit() %>%
    group_by(ID) %>% 
    summarise(maxT = CMaxTime(time, transT)) %>% 
    pull(maxT)
}
...
if(TRUE) # note this section can be computational intensive; don't run unless you have a cluster
{
  ...
  print(Sys.time())
  tmaxglobal = sobol2007(batch_CLPT, X1=samp[[1]], X2=samp[[2]], nboot=simulationboot) 
  plot(tmaxglobal)
  ...
  sobolx <- list(S = tmaxglobal$S , T = tmaxglobal$T)
  saveRDS(sobolx, file = "SobolData_CmaxTime.rds")
}
...
if(FALSE)
{
  ...
  x <- readRDS("SobolData_CmaxTime.rds")
  ### plot
  ...
  #png('GlobalSens_CLPT_CMaxTime.png', width = 800, height = 300)
  #print(fig_sens_glob)
  #dev.off()
}
