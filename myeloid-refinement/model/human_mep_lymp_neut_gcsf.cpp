...
The MK arm was adopted from thrombocytopenia model 
https://ghe.metrumrg.com/yuezhel/Thrombocytopenia-ADC/blob/main/model/platelet_dynamics_TPO1.jl

This file incorporate G-CSF impact on neutrophil developement (based on human_ery_lymp_mk_neutrophil.cpp)
Neutrophil development structure & parameters obtained from https://pubmed.ncbi.nlm.nih.gov/28303306/ & https://immunenetwork.org/DOIx.php?id=10.4110/in.2017.17.5.298

G-CSF dynamics obtained from https://linkinghub.elsevier.com/retrieve/pii/S0022519304005806
...
a_promyeloblast = 2.01 // base myeloblast proliferation time;  https://www.nature.com/articles/nature14242
tau_myelocyte = 5 // mean residence time of proliferating neutrophil progenitors, [day], tuned; https://pubmed.ncbi.nlm.nih.gov/28303306/
...
// G-SCF related dynamics 
abs_gcsf = 0.208 * 24 // absorption rate of subcutaneously administered G-CSF
gprod = 4.83 * 24 // endogenous G-CSF synthesis rate; [pM/day]
CLgcsf = 0.06 * 24 // nonspecific G-CSF clearance rate; [day-1]
k_on_gcsf_gcsfr = 0.015 * 24 // binding rate of G-CSF and G-CSF receptor; [pM-1.day-1]

beta4 = 0.2  // impact from G-CSF on promyelocytes amplification time; tuned
beta5 = 1  // impact from G-CSF on promyelocytes mean residence time; tuned
...
// CFU-MK dynamics
double aCFUMK = aCFUMK0 * log(2.73 + beta2 * TPO) / log(1.38); 
double tau1CFUMK = (log2(max(aCFUMK, 2.01))-1)/log2(aCFUMK) * tauCFUMK;
double tau2CFUMK = tauCFUMK - tau1CFUMK;
double krep1CFUMK = (aCFUMK/2-1)/ tau1CFUMK;
double krep2CFUMK = 1/ tau2CFUMK;
double k12CFUMK = aCFUMK/(2*tau1CFUMK);
double kCFUMK2MK = 2/tau2CFUMK;
...
// myelocyte dynamics
double A_promyeloblast = a_promyeloblast * max(1, beta4*gcsf); 
double tau_promyelocyte = tau_myelocyte/max(1, beta5*gcsf);

double tau1_promyelocyte = (log2(A_promyeloblast)-1)/log2(A_promyeloblast) * tau_promyelocyte;
double tau2_promyelocyte = tau_promyelocyte - tau1_promyelocyte;
double krep1_promyelocyte = (A_promyeloblast/2-1)/ tau1_promyelocyte;
double krep2_promyelocyte = 1/ tau2_promyelocyte;
double k12_promyelocyte = A_promyeloblast/(2*tau1_promyelocyte);
double k_promyelocyte2myelocyte = 2/tau2_promyelocyte;

double tau1_myelocyte = (log2(a_myelocyte)-1)/log2(a_myelocyte) * tau_myelocyte;
double tau2_myelocyte = tau_myelocyte - tau1_myelocyte;
double krep1_myelocyte = (a_myelocyte/2-1)/ tau1_myelocyte;
double krep2_myelocyte = 1/ tau2_myelocyte;
double k12_myelocyte = a_myelocyte/(2*tau1_myelocyte);
double k_myelocyte2metamyelocyte = 2/tau2_myelocyte;

// INIT condition of G-CSF receptor 
gcsfr_0 = 50; // [pM] 
...
dxdt_myeloblast = 2 * 1/tauGMP * GMP - 1/tau_myelocyte * myeloblast;

dxdt_promyelocyte01 = 2 * 1/tau_myelocyte * myeloblast + (krep1_promyelocyte - k12_promyelocyte) * promyelocyte01;
dxdt_promyelocyte02 = k12_promyelocyte * promyelocyte01 + (krep2_promyelocyte - k_promyelocyte2myelocyte) * promyelocyte02;

dxdt_myelocyte01 =  k_promyelocyte2myelocyte * promyelocyte02 + (krep1_myelocyte - k12_myelocyte) * myelocyte01; 
...
dxdt_neutrophil_blood = 1/tau_metamyelocyte * bandcell - betaGM * neutrophil_blood - k_pool_neutrophil * neutrophil_blood + k_pool_neutrophil * neutrophil_pool;
dxdt_neutrophil_pool = k_pool_neutrophil * neutrophil_blood - k_pool_neutrophil * neutrophil_pool;

// G-CSF dynamics 
dxdt_gcsfr = 0; // placeholder for G-CSF receptor
dxdt_gcsf = gprod - CLgcsf * gcsf - k_on_gcsf_gcsfr * gcsfr * gcsf + abs_gcsf * gcsf_depot / Vplasma;
dxdt_gcsf_depot = -abs_gcsf * gcsf_depot;
...
[CAPTURE]
aCFUE, sum_SP40, sum_SP80, k_myelocyte2metamyelocyte
