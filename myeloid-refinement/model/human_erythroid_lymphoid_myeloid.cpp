...
https://ghe.metrumrg.com/yuezhel/ex_vivo_gene_therapy
...
dxdt_BMspl1 = delta_t * Bt1 + phi_BM * BMrec1 - (phi_s + epsilon_spl_r) * BMspl1; 
...
dxdt_P01 = sum_mu_N1 + 2 * pN * N41 - (pP + delta_dp_r) * P01; 
dxdt_P11 = 2 * pP * P01 - ( pP + delta_dp_r + min( pow(alpha_muP * 1, n), 100) ) * P11; 
dxdt_P21 = 2 * pP * P11 - ( pP + delta_dp_r + min( pow(alpha_muP * 2, n), 100) ) * P21; 
dxdt_P31 = 2 * pP * P21 - ( pP + delta_dp_r + min( pow(alpha_muP * 3, n), 100) ) * P31; 
dxdt_P41 = 2 * pP * P31 - ( pP + delta_dp_r + min( pow(alpha_muP * 4, n), 100) ) * P41; 
dxdt_P51 = 2 * pP * P41 - ( pP + delta_dp_r + min( pow(alpha_muP * 5, n), 100) ) * P51; 
dxdt_P61 = 2 * pP * P51 - ( pP + delta_dp_r + min( pow(alpha_muP * 6, n), 100) ) * P61; 
