...
Model for cell dynamics healthy human (erythroid cells, hemoglobin, lymphocytes, myeloid); scaled up from mouse model

Erythroid arm from Zheng et al., 2021. 
https://ascpt.onlinelibrary.wiley.com/doi/10.1002/psp4.12638

Lymphocyte dynamics is taken from Busch et al., 2015
https://www.nature.com/articles/nature14242

T cell development in thymus is taken from Thomas-Vaslin et al., 2008
https://www.jimmunol.org/content/180/4/2240.long
  
Additional T cell dynamics in peripheral tissue and lymphoid system from Sove et al., 2020
https://ascpt.onlinelibrary.wiley.com/doi/10.1002/psp4.12546

B cell model is taken from Shahaf et al., 2016
https://www.frontiersin.org/articles/10.3389/fimmu.2016.00077/full

Adjustment on parameters
- CMP amplification number: 8 -> 36; the increase in CMP account for introducing myeloid branch; benchmark: aCFUE should be ~32 at steady state
- GMP amplification number: 32 -> 128; the increase is adjusted based on steady state GM should be 1-8k per uL blood;  
- B progenitor hosting capacity in bone marrow: 6M -> 8.4B; this is scaled based on cellular count of mouse bone marrow (500M) and human bone marrow (700B)

Adjustment on T cells
- separate naive T cell dynamics in lymph nodes and other peripheral tissues
- allow naive T cells to enter & leave lymph nodes and peripheral tissues with different rates
- naive T cells do not die in lymph nodes
- naive T cells (CD4+/ CD8+) are able to proliferate in peripheral tissues and blood
- universal death rate for naive T cells (either CD4+ or CD8+) in blood or other peripheral tissues
- CD4+ and CD8+ naive T cells have different rates exiting peripheral
...
alphaCLP2proB = 3 // differentiation rate, CLP -> preB; unit day-1
...
// note CLP2DN_amp bc steady state CD4+ output matches what is observed in Braber et al., 2012 in adults
// https://www.sciencedirect.com/science/article/pii/S1074761312000556
...
dxdt_CMP02 = k12CMP * CMP01 + (krep2CMP - kCMP2BFUE - kCMP2GMP) * CMP02; 

dxdt_CMP11 = kMPP2CMP * MPP12 + (krep1CMP - k12CMP) * CMP11;  
dxdt_CMP12 = k12CMP * CMP11 + (krep2CMP - kCMP2BFUE - kCMP2GMP) * CMP12; 
...
dxdt_cd4rec1 = sum_SP41 - death_naiveT_cd4 * cd4rec1 + exit_lymph * cd4lym1 - enter_lymph * cd4rec1 + exit_peripheral_cd4 * cd4peripheral1 - enter_peripheral * cd4rec1 + k_nT_pro/bloodvolume * (cd4rec1/bloodvolume)/(cd4rec1/bloodvolume + env_naiveT);
dxdt_cd8rec1 = sum_SP81 - death_naiveT_cd8 * cd8rec1 + exit_lymph * cd8lym1 - enter_lymph * cd8rec1 + exit_peripheral_cd8 * cd8peripheral1 - enter_peripheral * cd8rec1 + k_nT_pro/bloodvolume * (cd8rec1/bloodvolume)/(cd8rec1/bloodvolume + env_naiveT);
...
dxdt_cd4peripheral1 = k_nT_pro/peripheralvolume * (cd4peripheral1/peripheralvolume)/(cd4peripheral1/peripheralvolume + env_naiveT) - death_naiveT_cd4 * cd4peripheral1 - exit_peripheral_cd4 * cd4peripheral1 + enter_peripheral * cd4rec1;
dxdt_cd8peripheral1 = k_nT_pro/peripheralvolume * (cd8peripheral1/peripheralvolume)/(cd8peripheral1/peripheralvolume + env_naiveT) - death_naiveT_cd8 * cd8peripheral1 - exit_peripheral_cd8 * cd8peripheral1 + enter_peripheral * cd8rec1;
...
dxdt_cd4lym1 = enter_lymph * cd4rec1 - exit_lymph * cd4lym1;
dxdt_cd8lym1 = enter_lymph * cd8rec1 - exit_lymph * cd8lym1; 


[TABLE]
// progenitors in bone marrow
capture LTHSC = LT0 + LT1; // check whether LT-HSC is fully recovered
...
capture CLP = CLP0 + CLP1;
capture BprogBM = Boe0 + Boe1 + Bi0 + Bi1;
...
capture SP4thymus = S400 + S410 + S420 + S401 + S411 + S421;
capture SP8thymus = S800 + S810 + S820 + S801 + S811 + S821;

// B cells in spleen
capture splenicB = Bt0 + BMspl0 + Bt1 + BMspl1;

// T cells in other ograns
capture nT_lymphnodes = cd4lym0 + cd8lym0 + cd4lym1 + cd8lym1;
capture nT_peripheral = cd4peripheral0 + cd8peripheral0 + cd4peripheral1 + cd8peripheral1;
...
capture Bconc = (BMrec0 + BMrec1)/(bloodvolume * 1e6); // naive B per uL
capture Tconc = (cd4rec0 + cd4rec1 +  cd8rec0 + cd8rec1)/(bloodvolume * 1e6); // naive T per uL
capture GMconc = (GM0 + GM1)/(bloodvolume * 1e6); // granulocytes per uL
...
// fluxes
capture CLP2proB0 = CLP2proB_amp * alphaCLP2proB * CLP0 + CLP2proB_amp * alphaCLP2proB * CLP1; // CLP -> propreB cells; 
capture CLPexport2thymus = alphaCLP2DN * (CLP0 + CLP1); 

// fluxes related to thymus
capture thymic_output = sum_SP40 + sum_SP80 + sum_SP41 + sum_SP81;
capture naiveTprof = k_nT_pro/peripheralvolume * (cd4peripheral0/peripheralvolume)/(cd4peripheral0/peripheralvolume + env_naiveT) + 
                     ...
                     k_nT_pro/peripheralvolume * (cd4peripheral1/peripheralvolume)/(cd4peripheral1/peripheralvolume + env_naiveT) + 
                     k_nT_pro/peripheralvolume * (cd8peripheral1/peripheralvolume)/(cd8peripheral1/peripheralvolume + env_naiveT) + 
                     ...
                     k_nT_pro/bloodvolume * (cd4rec1/bloodvolume)/(cd4rec1/bloodvolume + env_naiveT) + 
                     k_nT_pro/bloodvolume * (cd8rec1/bloodvolume)/(cd8rec1/bloodvolume + env_naiveT) ; 
                     
// transduced cell fraction
capture transT = (cd4rec1 + cd8rec1)/ (cd4rec0 + cd4rec1 +  cd8rec0 + cd8rec1);
capture transB = BMrec1/ (BMrec0 + BMrec1);

[CAPTURE]
aCFUE, sum_SP40, sum_SP80, sum_SP41, sum_SP81
