...
model adapted from Zheng et al., 2021. 
https://ascpt.onlinelibrary.wiley.com/doi/10.1002/psp4.12638

Changes: introduce a new parameter to model the decrease in endogenous beta globin synthesis;; 
this change is placeholder for beta thalassemia disease state
...
// add another parameter to account for decrease beta globin synthesis in patients
// this is the ratio that the endogenous beta globin synthesis is impacted; this number should be between 0 and 1
E_synbeta_effect = 0.8
...
dxdt_beta_RET0  = ksynbeta * E_synbeta_effect * RET0 - (kdeg + kRET2RBC)*beta_RET0  - kon * alpha_RET0 * beta_RET0  + koffalphabetasickle *VRET0 * alphabeta_RET0  ; 
...
dxdt_beta_RET1    = ksynbeta * (1-ratiosynbetanew) * E_synbeta_effect * RET1 - (kdeg + kRET2RBC)* beta_RET1    - kon*alpha_RET1*beta_RET1    + koffalphabetasickle *VRET1 * alphabeta_RET1; 
...
dxdt_beta_RET2    = ksynbeta * (1-ratiosynbetanew) * E_synbeta_effect * RET2 - (kdeg + kRET2RBC)* beta_RET2    - kon*alpha_RET2*beta_RET2    + koffalphabetasickle *VRET2 * alphabeta_RET2 ; 
...
dxdt_beta_RET3    = ksynbeta * (1-ratiosynbetanew) * E_synbeta_effect * RET3 - (kdeg + kRET2RBC)* beta_RET3    - kon*alpha_RET3*beta_RET3    + koffalphabetasickle *VRET3* alphabeta_RET3 ; 
...
dxdt_beta_RET4    = ksynbeta * (1-ratiosynbetanew) * E_synbeta_effect * RET4 - (kdeg + kRET2RBC)* beta_RET4    - kon*alpha_RET4*beta_RET4    + koffalphabetasickle *VRET4* alphabeta_RET4; 
...
capture RBCconc = (RBC0 + RBC1 + RBC2 + RBC3 + RBC4)/ (bloodvolume * 1e6); // RBC per uL
