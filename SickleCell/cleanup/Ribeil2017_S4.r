...
RBCnum = ggplot(data = sim, aes(x = time/30)) + 
  geom_line(aes(y = RBCconc, color = 'total RBC')) +
  geom_line(aes(y = RBC0/5e6, color = 'endogenous RBC')) +
  geom_line(aes(y = RBC1/5e6, color = 'transduced RBC')) +
...
RETnum = ggplot() + 
  ...
  geom_line(data = sim, aes(x = time/30, y = RET1/(5e6), color = 'transduced RET')) + 
...
png(filename="Ribeil2017-S4.png", width = 200, height = 250, unit = "mm", res = 200)
